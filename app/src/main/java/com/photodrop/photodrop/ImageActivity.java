...
public class ImageActivity extends AppCompatActivity implements ValueEventListener, View.OnClickListener {
    ...
    public void setImageData(String imageKey) {
        ...
        // TODO: Should probably save the OTHER user's images locally and check if the image is
        // saved before accessing the DB again. But need to differentiate between the user's photos
        // and others' photos
        ...
        File photoFile = new File(photodropDir, imageKey + MainActivity.FILE_EXTENSION);

        // If the photo exists, then show the local version instead!
        if (photoFile.exists()) {
            Log.d("ME", photoFile.getName() + " exists!");
...
//                    return ImageUtil.getBitmapFromUri(ImageActivity.this, params[0]);
                ...
                protected void onPostExecute(Bitmap bitmap) {
                    ...
                    Log.d("ME", "onPostExecute() in ImageActivity");
                    imageView.setImageBitmap(bitmap);
                    // Hides the progress bar
                    showProgress(false);
                }
            }.execute(Uri.fromFile(photoFile));
        ...
        // Adds a listener then removes it once it's triggered so that the num likes is set
